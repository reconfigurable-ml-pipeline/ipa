...
try:
    ...
    NUM_THREADS = 1
    logger.info(
        f"NUM_THREADS env variablke not set, using default value: {NUM_THREADS}"
    )
...
class GeneralNLP(MLModel):
    async def load(self):
        ...
        try:
            ...
            self.MODEL_VARIANT = "Helsinki-NLP/opus-mt-fr-en"
            logger.error(
        ...
        try:
            ...
            self.TASK = "translation_fr_to_en"
            logger.error(
    ...
    async def predict(self, payload: InferenceRequest) -> InferenceResponse:
        ...
        if type(output) == dict:
            output = [output]
        output = list(map(lambda l: str(l["translation_text"]), output))
