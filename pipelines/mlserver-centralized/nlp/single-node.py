...
def load_test(
    ...
    for iter in range(n_iters):
        response = sc.predict(str_data=inputs)
        ...
        if response.success:
            ...
            for i, name in enumerate(return_nodes):
                ...
                memory_usages[i].append(
                    get_memory_usage(pipeline_name, "default", name, 1)
                )
    ...
    for i, name in enumerate(return_nodes):
        cpu_usages[i].append(get_cpu_usage(pipeline_name, "default", name))
        memory_usages[i].append(
            get_memory_usage(pipeline_name, "default", name, total_time, True)
        )
    models = node_1_model
...
def setup_pipeline(node_1_model: str, template: str, pipeline_name: str):
    svc_vars = {
    ...
    environment = Environment(loader=FileSystemLoader(os.path.join(PATH, "templates/")))
...
config_file_path = os.path.join(PIPELINES_MODELS_PATH, f"{CONFIG_FILE}.yaml")
...
for node_1_model in node_1_models:
    ...
    while True:
        setup_pipeline(
            node_1_model=node_1_model, template=TEMPLATE, pipeline_name=pipeline_name
        )
        time.sleep(CHECK_TIMEOUT)
        command = (
            "kubectl rollout status deploy/$(kubectl get deploy"
